...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    staging_redacted_transcript_blob.upload_blob(
        data=dumps(redacted_transcript_json), content_type="application/json"
    )
    staging_redacted_nlp_blob.upload_blob(
        data=dumps(redacted_nlp_json), content_type="application/json"
    )
    ### Confirm uploaded
    ...
    for media_type, media_file in [
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with open(redacted_media_temp_file_path, "rb") as f:
                staging_redacted_media_blob.upload_blob(f, timeout=300)
